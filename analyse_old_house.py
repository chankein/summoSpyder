...
class do_caltulate:
    ...
    def regression(self):
        ...
        df_target=pd.concat([self.df_target,df_split1,df_split2], axis=1)
        df_target['距離(分)']=df_target['距離'].apply(lambda x: get_num(x))
        df_regression=df_target.loc[:,['建物面積(m2)','土地面積(m2)','築年数','駅','距離(分)','販売価格(万円)']]
        X=df_regression.loc[:,['建物面積(m2)','土地面積(m2)','築年数','距離(分)']]
        y=df_regression['販売価格(万円)']
